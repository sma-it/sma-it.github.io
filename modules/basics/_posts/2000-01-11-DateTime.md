...
```csharp
DateTime startDate = new DateTime(2020, 9, 1, 23, 50, 25);  //ToDo: dateTimeKind local bespreken
Console.WriteLine(startDate.ToString());
...
Voorbeeld: De variabele `dateNow` wordt via de property `Now` geïnitialiseerd op de huidige tijd van de computer (in de lokale tijdzone). 
Er zijn verschillende methoden om de opgegeven datum naar UTC te converteren:
* Door op deze variabele de method `ToUniversalTime()` uit de `DateTime` class toe te passen.
* Door op deze variabele de method `ConvertTimeToUtc()` uit de `TimeZoneInfo` class toe te passen.

```csharp
DateTime dateNow = DateTime.Now;
Console.WriteLine("The local date and time: {0}", dateNow);
Console.WriteLine("The date and time converted to UTC: {0}", dateNow.ToUniversalTime());

/* Uitvoer:
The local date and time: 12/08/2018 16:55:56
The date and time converted to UTC: 12/08/2018 14:55:56
*/
```
...
DateTime dateNow = DateTime.Now;
Console.WriteLine("The local date and time: {0}", dateNow);
Console.WriteLine("The date and time converted to UTC: {0}", TimeZoneInfo.ConvertTimeToUtc(dateNow));
...
Op het eerste zicht zijn beide bovenstaande methodes gelijkwaardig. Toch is er een belangrijk verschil. Waar de method `TimeZoneInfo.ConvertTimeToUtc()` rekening houdt met onbestaande momenten, doen niet alle versies van `DateTime.ToUniversalTime()` dit. Dit kan problemen geven in gebieden waar Daylight Savings Time actief is. Zo zal er op die plaatsen, bij overgang naar het zomeruur, de tijdspanne tussen 2 en 3 uur niet bestaan. Waar `dateNow.ToUniversalTime()` nog steeds de omzetting zal doen van deze onbepaalde tijd, geeft `TimeZoneInfo.ConvertTimeToUtc(dateNow)` volgende exception: `De opgegeven DateTime vertegenwoordigt een ongeldige tijd. Als de klok bijvoorbeeld voorwaarts wordt aangepast, is elke tijd in de overgeslagen periode ongeldig`.
