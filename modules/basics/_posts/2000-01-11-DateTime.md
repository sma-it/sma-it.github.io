...
Een DateTime kan een datum en tijd voorstellen tussen 1 januari 0001 om 00:00:00 en 31 december 9999 om 23:59:59.9999999.

Je vindt een beschrijving van de `DateTime` class onder volgende [link](https://learn.microsoft.com/dotnet/api/system.datetime)
...
Op het eerste gezicht lijkt het gebruik van datum en tijd vrij vanzelfsprekend. Toch zijn er een aantal onverwachte valkuilen. In wat volgt zie je hoe je deze kan vermijden.
...
Opmerking: door op het DateTime object de method `ToString()` toe te passen kan het formaat waarin de datum en tijd op het scherm getoond worden beïnvloed worden. Indien je een ander formaat wil tonen dan wat er standaard als uitvoer gebruikt wordt, pas je dus de `ToString()` method op het DateTime-object toe met vermelding van de gewenste paramters. In de link met de documentatie vind je hierover meer info.
...
Console.WriteLine(startDate);
Console.WriteLine($"This is a {startDate.DayOfWeek}");
/* De uitvoer is afhankelijk van de landinstellingen van de computer.
Landinstelling met notatie Nederlands/België:
1/09/2020 23:50:25
This is a Tuesday
...
Console.WriteLine($"The local date and time: {dateNow}");
Console.WriteLine($"Converted to UTC: {dateNow.ToUniversalTime()}");
...
DateTime dateNow = DateTime.Now;
Console.WriteLine($"The local date and time: {dateNow}");
Console.WriteLine($"Converted to UTC: {TimeZoneInfo.ConvertTimeToUtc(dateNow)}");
...
Met onderstaande functie kan je het verschil tussen beide methods uittesten en de exception genereren. Er wordt een instantie van `DateTime` gemaakt die niet bestaat, namelijk kwart na twee tijdens de overgang van winter- naar zomertijd.
...
Console.WriteLine($"The UTC-tijd: {timeUtc}");
Console.WriteLine($"Converted to local time: {localTime}");
...
Console.WriteLine($"The timezone of date1: {date1.Kind}");
Console.WriteLine($"The timezone of date2: {date2.Kind}");
Console.WriteLine($"The timezone of date3: {date3.Kind}");
...
Met de method `SpecifyKind` kan je property `Kind` instellen, zoals je in onderstaand voorbeeld ziet. Opmerking: `SpecifyKind()` verandert de tijd niet. Enkel de waarde van de property Kind wordt aangepast.
