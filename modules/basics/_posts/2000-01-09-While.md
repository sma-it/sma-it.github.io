...
## Algemene syntax van het do while statement
...
Zoals reeds vermeld in het voorgaande wordt het instructieblok bij `do while` minstens één maal uitgevoerd. Een typisch voorbeeld van het gebruik van een `do while` statement vind je bij invoercontrole van gebruikersinvoer in een consoleprogramma. Maar ook daar kan je door variabelen op voorhand te initialiseren, wat C# sowieso vraagt, een `do while` statement schrijven als een `while` statement.

Het `do while` statement wordt dus niet verder behandeld in deze cursus.
