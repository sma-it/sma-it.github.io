...
---
<div class="header1" id="top" markdown = "1"># Lus met While
</div>
...
De functie bepaalt hoe vaak een getal deelbaar is door 2 voordat dit getal kleiner wordt dan 1. We gaan ervanuit dat de beginwaarde voor de functieparameter number steeds groter is dan 1.

```csharp
public static int CountDivisionsUntilBelowOne(float number)
{
    int count = 0;
    while (number > 1)
    {
...
* Het instructieblok: Het nummer wordt gelijkgesteld aan zichzelf gedeeld door 2 en de teller count wordt met 1 verhoogd. Door het nummer in de lus te wijzigen, wijzigt ook de voorwaarde van de while-lus. Het is essentieel om de voorwaarde in de lus te wijzigen om een oneindige lus te vermijden.
...
Soms kan het handig zijn om de voorwaarde voor het beëindigen van de lus niet bij `while` te vermelden, maar de lus te stoppen d.m.v. een `break` binnen de lus zelf. Een while(true)-lus wordt meestal gebruikt wanneer het stopmoment pas tijdens de uitvoering duidelijk wordt. Gebruik dit alleen wanneer een break de lus duidelijk beëindigt. Onthoud dat dit niet de standaard manier is om een while-lus te schrijven en dat je dit enkel gebruikt waar nodig.
...
<div class="header2" markdown = "1">## Lus met do while
</div>
